import 'dart:typed_data';

import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/foundation.dart';
import 'package:project_with_android/model/user.dart';
import 'package:project_with_android/reponsive/storage_method.dart';
import '../model/post.dart';

class FirestoreMethod {
  final _firestore = FirebaseFirestore.instance;

  Future<String> uploadPost({required String description, required Uint8List file, required String uid}) async {
    var res = "Some error occured";
    try {
      String photoUrl = await StorageMethod().uploadImagetoStorage('Posts', file, true);
      Post post = Post(
        description: description,
        uid: uid,
        postUrl: photoUrl,
        datePublished: DateTime.now(),
        likes: [],
        likesCount: 0,
      );

      _firestore.collection('Posts').add(post.toJson());

      res = 'Success';
    } on FirebaseException catch (err) {
      print(['error', err]);
      res = err.toString();
    } catch (err) {
      res = err.toString();
    }
    return res;
  }

  Future<User?> getUser(String uid) async {
    try {
      var data = await _firestore.collection('users').doc(uid).get();

      return User.fromSnap(data);
    } on FirebaseException catch (err) {
      print(['error', err]);
    } catch (err) {
      print(['error', err]);
    }
  }

  Future<String> likePost(String postId, String uid, List likes, int likesCount) async {
    String res = "Some error occurred";
    try {
      if (likes.contains(uid)) {
        // if the likes list contains the user uid, we need to remove it
        _firestore.collection('Posts').doc(postId).update({
          'likes': FieldValue.arrayRemove([uid]),
          'likesCount': likesCount - 1,
        });
      } else {
        // else we need to add uid to the likes array
        _firestore.collection('Posts').doc(postId).update({
          'likes': FieldValue.arrayUnion([uid]),
          'likesCount': likesCount + 1,
        });
      }
      res = 'success';
    } catch (err) {
      res = err.toString();
    }
    return res;
  }

  Future<String> postComment(String postId, String text, String uid, String name, String profilePic) async {
    String res = "Some error occurred";
    try {
      if (text.isNotEmpty) {
        // if the likes list contains the user uid, we need to remove it
        _firestore.collection('Posts').doc(postId).collection('comments').add({
          'profilePic': profilePic,
          'name': name,
          'uid': uid,
          'text': text,
          'datePublished': DateTime.now(),
        });
        res = 'success';
      } else {
        res = "Please enter text";
      }
    } catch (err) {
      res = err.toString();
    }
    return res;
  }

  Future<void> followUser(String uid, String followId) async {
    try {
      DocumentSnapshot snap =
      await _firestore.collection('users').doc(uid).get();
      List following = (snap.data()! as dynamic)['following'];

      if (following.contains(followId)) {
        await _firestore.collection('users').doc(followId).update({
          'followers': FieldValue.arrayRemove([uid])
        });

        await _firestore.collection('users').doc(uid).update({
          'following': FieldValue.arrayRemove([followId])
        });
      } else {
        await _firestore.collection('users').doc(followId).update({
          'followers': FieldValue.arrayUnion([uid])
        });

        await _firestore.collection('users').doc(uid).update({
          'following': FieldValue.arrayUnion([followId])
        });
      }
    } catch (e) {
      if (kDebugMode) print(e.toString());
    }
  }
}
